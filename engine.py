...
def handle_lksfy(key_url, session, verify, debug):
    ...
    try:
        ...
        response = session.get(key_url, headers={"referer": key_url}, timeout=30,
        ...
        if response.status_code in (301, 302, 303, 307, 308):
            ...
            # Make the second request with referer
            headers = {"referer": redirect_url}
            dbg(f"Lksfy request 2 -> {key_url} with referer", debug)
            second_response = session.get(key_url, headers=headers, timeout=30,
                                          verify=verify)
            ...
            if second_response.status_code == 200:
                ...
                if not base64_match:
                    return None, key_url, RuntimeError("Base64 value not found in HTML")
                ...
                if not decrypted_html:
                    return None, key_url, RuntimeError("Failed to decrypt base64 value")
                ...
                post_response = session.post(post_url, headers=post_headers,
                                             data=post_body, timeout=30, verify=verify)
                dbg(f"Lksfy request 3 status: {post_response.status_code}", debug)

                if post_response.status_code != 200:
                    return None, key_url, RuntimeError("POST request failed")

                # Parse JSON
                try:
                    json_response = post_response.json()
                except:
                    return None, key_url, RuntimeError("Failed parsing JSON")

                if json_response.get("status") != "success":
                    return None, key_url, RuntimeError(f"Server error: {json_response}")

                encrypted_url = json_response.get("url")
                decrypted_url = decrypt(encrypted_url, alias, debug)
                if not decrypted_url:
                    return None, key_url, RuntimeError("Failed to decrypt final URL")

                dbg(f"Final URL: {decrypted_url}", debug)

                # -------------------------------------------------
                # 1️⃣ Normal format → key=XXXXXX
                # -------------------------------------------------
                ...
                # -------------------------------------------------
                # 2️⃣ /go/ encrypted redirect support
                # -------------------------------------------------
                if decrypted_url.startswith("/go/"):
                    encoded = decrypted_url.replace("/go/", "").strip()
                    try:
                        step1 = base64.b64decode(encoded + "===")
                        step2 = base64.b64decode(step1[::-1])
                        key_match = re.search(r"key=([^&]+)", step2.decode())
                        if key_match:
                            key = key_match.group(1)
                            ok(f"Extracted key: {key}")
                            return key, None, None
                    except Exception as e:
                        dbg(f"Go-link decode error: {e}", debug)

                # -------------------------------------------------
                # 3️⃣ TELEGRAM bot links → ?start=verify_XXXX
                # -------------------------------------------------
                try:
                    parsed_final = urlparse(decrypted_url)
                    qs = parse_qs(parsed_final.query)

                    if "start" in qs:
                        start_val = qs["start"][0]

                        # If format is verify_XXXX → extract only XXXX
                        if start_val.startswith("verify_"):
                            key = start_val.split("verify_", 1)[1]
                        else:
                            key = start_val
                    ...
                    dbg(f"Start-param parse error: {e}", debug)

                # -------------------------------------------------
                # Nothing matched
                # -------------------------------------------------
                return None, key_url, RuntimeError("Key not found in final URL")

            else:
                return None, key_url, RuntimeError("Second GET failed")

        else:
            return None, key_url, RuntimeError("Initial redirect missing")
...
def main(user_link):
    ...
    print("[INFO] Processing your link...")

    try:
        if "lksfy.com" in user_link:
            key, bad_url, error = handle_lksfy(user_link, session, verify, debug)
        elif "nanolinks" in user_link:
            key, bad_url, error = handle_nano_links(user_link, session, verify, debug)
        elif "arolinks" in user_link:
            key, bad_url, error = handle_aro_links(user_link, session, verify, debug)
        else:
            err("Unsupported link! Only lksfy/nanolinks/arolinks supported.")
            sys.exit(1)

        if key:
            ok(f"Your KEY: {key}")
        else:
            err(f"Failed to extract key from: {bad_url}")
            if error:
